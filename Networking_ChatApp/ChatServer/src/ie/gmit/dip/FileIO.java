...
public class FileIO {
	...
	private String readConfigFile(String filePath) {
		...
		try (
				...
				//InputStream is = this.getClass().getClassLoader().getParent().getResourceAsStream("\\config.txt");
	...
	public List<String> getConfigInfo(String filePath) {
		...
		if (line.length() > 0) {
			try {
				...
				System.out.println("[ERROR] Problem encountered reading data from config file.");
			}
		}
		return configInfo;
	}
